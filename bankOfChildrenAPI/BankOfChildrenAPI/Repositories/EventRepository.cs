...
namespace BankOfChildrenAPI.Repositories
{
    ...
    public interface IDbCollectionEventRepository<TModel, in TPk>
    {
    ...
    /// </summary>
    public class EventCollectionOperationsRepository : IDbCollectionEventRepository<Event, string>
    {
        ...
        private static readonly string DatabaseId = "zone";
        private static readonly string CollectionId = "Events";
        private static DocumentClient docClient;
        #endregion

        public EventCollectionOperationsRepository()
        {
        ...
        /// <returns></returns>
        public async Task<Event> AddDocumentIntoCollectionAsync(Event item)
        {
            try
            {   
                ...
                var person = JsonConvert.DeserializeObject<Event>(res.ToString());
                return person;
            }
            catch (Exception ex)
            {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<Event> GetItemFromCollectionAsync(string id)
        {
            try
            {
                Document doc = await docClient.ReadDocumentAsync(UriFactory.CreateDocumentUri(DatabaseId, CollectionId, id));
                return JsonConvert.DeserializeObject<Event>(doc.ToString()); 
            }
            catch (DocumentClientException e)
            {
                if (e.StatusCode == System.Net.HttpStatusCode.NotFound)
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<IEnumerable<Event>> GetItemsFromCollectionAsync()
        {
            var documents = docClient.CreateDocumentQuery<Event>(
                  UriFactory.CreateDocumentCollectionUri(DatabaseId, CollectionId),
                  new FeedOptions { MaxItemCount = -1 })
                  .AsDocumentQuery();
            List<Event> persons = new List<Event>();
            while (documents.HasMoreResults)
            { 
                persons.AddRange(await documents.ExecuteNextAsync<Event>());
            }
            return persons;
        }   
        /// <summary>
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public async Task<Event> UpdateDocumentFromCollection(string id, Event item)
        {
            try
            {
                ...
                var person = JsonConvert.DeserializeObject<Event>(data);
                return person;
            }
            catch (Exception ex)
            {
